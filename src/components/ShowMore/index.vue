<template>
  <div class="more-box" :style="{ paddingBottom: isLongContent?'40px':'0'}">
  ...
  export default {
    ...
    methods: {
      _calculateHeight () {
        ...
        this.$nextTick().then(() => {
          let contentHeight = that.$refs.contentbox.clientHeight
          if (contentHeight > that.showHeight) {
            that.isLongContent = true
          } else {
            that.isLongContent = false
          }
        })
      },
      _toggleShowMore () {
